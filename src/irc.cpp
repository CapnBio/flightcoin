...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            ...
            // randomly join #flightcoin00-#flightcoin99
            int channel_number = GetRandInt(100);
            channel_number = 0; // Litecoin: for now, just use one channel
            Send(hSocket, strprintf("JOIN #flightcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #flightcoin%02d\r", channel_number).c_str());
        }
